...
const BookRecommendationApp: React.FC = () => {
  ...
  const getRecommendations = async () => {
    ...
    const sortedBooks = filteredBooks.sort((a, b) => {
      const aRelevance = a.genres.filter(genre => selectedGenres.includes(genre)).length +
                         (a.mood.includes(selectedMood) ? 1 : 0);
      const bRelevance = b.genres.filter(genre => selectedGenres.includes(genre)).length +
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="mb-4">
        <label htmlFor="mood" className="block mb-2 font-semibold">Select your current mood:</label>
        <select
          id="mood"
          value={selectedMood}
